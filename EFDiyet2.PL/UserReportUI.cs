...
namespace EFDiyet2.PL
{
    public partial class UserReportUI : Form
    {
        ...
        private void dateTimePicker1_ValueChanged_1(object sender, EventArgs e)
        {
            ...
            var filtered = userFoodManager.GetAll().Where(e => e.UserInformationId == userLogedin && e.UCreated.Day == dateTimePicker1.Value.Day && e.UCreated.Month == dateTimePicker1.Value.Month && e.UCreated.Year == dateTimePicker1.Value.Year).ToList();
            dgvUserreport.DataSource = filtered;
            dgvUserreport.Columns[0].Visible = false;
            dgvUserreport.Columns[1].Visible = false;
            dgvUserreport.Columns[3].Visible = false;
            dgvUserreport.Columns[4].Visible = false;
        ...
        {
            var filtered = userFoodManager.GetAll().Where(e => e.UserInformationId == userLogedin && e.UCreated.Day == dateTimePicker1.Value.Day && e.UCreated.Month == dateTimePicker1.Value.Month && e.UCreated.Year == dateTimePicker1.Value.Year).ToList();
            lblCalorie.Text = "Total Calorie: " + filtered.Sum(x => x.Food.Calorie).ToString();
            lblTotalCarb.Text = "Total Carbonhydrate: " + filtered.Sum(x => x.Food.Carbohydrate).ToString();
            lblTotalProtein.Text = "Total Protein: " + filtered.Sum(x => x.Food.Protein).ToString();
            lblTotalFiber.Text = "Total Fiber: " + filtered.Sum(x => x.Food.Fiber).ToString();
            lblTotalFat.Text = "Total Fat: " + filtered.Sum(x => x.Food.Fat).ToString();
        }
    }
}
