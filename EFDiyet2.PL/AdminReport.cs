...
namespace EFDiyet2.PL
{
    public partial class AdminReport : Form
    {
        ...
        private void btnGetReport_Click(object sender, EventArgs e)
        {
            ...
            int selectedUser = ((UserInformationModel)cboxUser.SelectedItem).Id;
            ...
            var DateStart = userFoodManager.GetAll().Where(e => e.UserInformationId == selectedUser && DateOnly.FromDateTime(e.UCreated) >= DateOnly.FromDateTime(dtvStart.Value)).ToList();

            var DateFiltered = DateStart.Where(e => DateOnly.FromDateTime(e.UCreated) <= DateOnly.FromDateTime(dtvEnd.Value)).ToList();



            lblCalorie.Text = "Total Calorie: " + DateFiltered.Sum(x => x.Food.Calorie).ToString();
            lblTotalCarb.Text = "Total Carbonhydrate: " + DateFiltered.Sum(x => x.Food.Carbohydrate).ToString();
            lblTotalProtein.Text = "Total Protein: " + DateFiltered.Sum(x => x.Food.Protein).ToString();
            lblTotalFiber.Text = "Total Fiber: " + DateFiltered.Sum(x => x.Food.Fiber).ToString();
            lblTotalFat.Text = "Total Fat: " + DateFiltered.Sum(x => x.Food.Fat).ToString();

            var consumeFoods = DateFiltered.GroupBy(userFood => userFood.Food.Name).Select(group => new { FoodName = group.Key, TotalConsumption = group.Sum(userFood => userFood.Piece) }).OrderByDescending(item => item.TotalConsumption).ToList();

            //var TopFoodonMealss = DateFiltered.SelectMany(meal => meal.Food.Name, (meal, food) => new { Meal = meal, Food = food })
            //.GroupBy(x => x.Meal);




            dgvFoodReport.DataSource = DateFiltered;

            dgvFoodReport.Columns[0].Visible = false;
            dgvFoodReport.Columns[1].Visible = false;
            dgvFoodReport.Columns[3].Visible = false;
            dgvFoodReport.Columns[4].Visible = false;
            dgvFoodReport.Columns[6].Visible = false;
            dataGridView1.DataSource = consumeFoods;
